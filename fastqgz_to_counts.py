...
def seqFileToCounts(infileName, fastaFileName, countFileName, libraryFasta, startIndex=None, stopIndex=None,
    ...
    for fileTup in acceptedFileTypes:
        if fnmatch.fnmatch(infileName, fileTup[0]):
            fileType = fileTup[1]  # convert fileUp[0] to fileUp[1]
            break
    ...
    with open(fastaFileName, 'w') as unalignedFile:
        for i, fastqLine in enumerate(infile):
            if i % linesPerRead != 1:  # only deal with sequence line
                continue

            else:
                # from 1 to 35 in each sequencing fastq file
...
if __name__ == '__main__':
    # fastqgz_to_counts.py -p 6 --trim_start 1 --trim_end 35
    # library_reference/CRISPRi_v1_human.trim_1_35.fa
    # Demo/Step1/output
    # Demo/Step1/Sequencing_files/Demo*
    ...
    parser.add_argument('Seq_File_Names', nargs='+',
                        help='Name(s) of sequencing file(s). Unix wildcards can be used to select multiple files at once. '
                             'The script will search for all *.fastq.gz, *.fastq, '
                             'and *.fa(/fasta/fna) files with the given wildcard name.')
    ...
    parser.add_argument('--test', action='store_true', default=False,
                        help='Run the entire script on only the first %d reads of each file. '
                             'Be sure to delete or move all test files before re-running script '
                             'as they will not be overwritten.' % testLines)
    ...
    try:
        # expectedReadLength is the length of sequence in library file
